...
def get_nearest_neighbors(users_collection, user_id, month, year, category):
...
def get_prediction_by_category(train_data, users_outcome_per_category_dic, current_user_data, category, only_distances=False):
    train_res = []
    ...
    if only_distances:
        distances, indices = neigh.kneighbors(current_user_data)
        return indices
